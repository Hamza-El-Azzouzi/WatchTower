...
fn test_env_override_retention_config() {
    env::set_var("RETENTION_METRICS_HOURS", "48");
...
fn test_env_override_invalid_values() {
    ...
    env::set_var("RETENTION_METRICS_HOURS", "not_a_number");
    ...
    assert_eq!(config.server.port, 8080);
    assert_eq!(config.retention.metrics_hours, 720);
...
fn test_default_configs() {
    ...
    assert_eq!(db_config.url, "sqlite:monitoring.db");

    let retention_config = RetentionConfig::default();
    assert_eq!(retention_config.metrics_hours, 720);
    assert_eq!(retention_config.alerts_days, 90);
    assert_eq!(retention_config.cleanup_interval_hours, 24);
